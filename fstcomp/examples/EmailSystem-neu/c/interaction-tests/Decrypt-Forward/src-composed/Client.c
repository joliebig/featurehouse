...
{ encrypt (client, msg); outgoing__wrappee__Base(client, msg);
}
...
{ decrypt (client, msg); incoming__wrappee__Encrypt(client, msg);
}

void
incoming (struct client *client, struct email *msg)
{ forward (client, msg); incoming__wrappee__Decrypt(client, msg);
}
// adds the encrypt flag to message body
void
encrypt (struct client *client, struct email *msg)
{ int i; for (i = 0; i < (*client).keysSize; i++) { if (strncmp (((*client).keys + i * MAX), (*msg).to, strlen ((*msg).to)) == 0)	{ printf ("> encrypted\n"); char newBody[MAX] = "[encrypted]\n"; strcat (newBody, (*msg).body); strcpy ((*msg).body, newBody); break;	} }
}
