...
public  class  LogEntryType {
	 ...
	 public static boolean isNodeType( byte typeNum, byte version){ return (typeNum <= MAX_NODE_TYPE_NUM); }
	 ...
	 public boolean isNodeType(){ return (typeNum <= MAX_NODE_TYPE_NUM); }

	 public static LogEntryType findType( byte typeNum, byte version){ if (typeNum <= 0 || typeNum > MAX_TYPE_NUM) { return null; } return (LogEntryType)LOG_TYPES[typeNum - 1]; }

	 public static Set getAllTypes(){ HashSet ret=new HashSet(); for (int i=0; i < MAX_TYPE_NUM; i++) { ret.add(LOG_TYPES[i]); } return ret; }

	 public LogEntry getSharedLogEntry(){ return logEntry; }

	 LogEntry getNewLogEntry() throws DatabaseException { try { return (LogEntry)logEntry.clone(); } catch ( CloneNotSupportedException e) { throw new DatabaseException(e); } }

	 static byte setProvisional( byte version){ return (byte)(version | PROVISIONAL_MASK); }

	 public static byte clearProvisional( byte version){ return (byte)(version & IGNORE_PROVISIONAL); }

	 static boolean isProvisional( byte version){ return ((version & PROVISIONAL_MASK) != 0); }

	 byte getTypeNum(){ return typeNum; }

	 byte getVersion(){ return version; }

	 static boolean isValidType( byte typeNum){ return typeNum > 0 && typeNum <= MAX_TYPE_NUM; }

	 public String toString(){ return displayName + "/" + version; }

	 boolean equalsType( byte typeNum, byte version){ return (this.typeNum == typeNum); }

	 public boolean equalsType( byte typeNum){ return (this.typeNum == typeNum); }

	 public boolean equals( Object obj){ if (this == obj) { return true; } if (!(obj instanceof LogEntryType)) { return false; } return typeNum == ((LogEntryType)obj).typeNum; }

	 public int hashCode(){ return typeNum; }
