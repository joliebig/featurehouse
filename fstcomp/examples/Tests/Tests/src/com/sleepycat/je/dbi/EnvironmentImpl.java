...
@MethodObject static  class  EnvironmentImpl_createDaemons {
		 ...
		 void execute() throws DatabaseException { checkpointerWakeupTime=0; this.hook329(); _this.checkpointer=new Checkpointer(_this,checkpointerWakeupTime,"Checkpointer"); this.hook332(); _this.cleaner=new Cleaner(_this,"Cleaner"); }
	 ...
	 protected void hook321() throws DatabaseException { logManager=new SyncedLogManager(this,isReadOnly); }

	 protected void hook322() throws DatabaseException { }

	 protected void hook323() throws DatabaseException { }

	 protected void hook324( long rootLsn) throws DatabaseException { mapTreeRootLsn=rootLsn; }
	 ...
	 protected void hook336__wrappee__BBase( File envHome) throws DatabaseException { }

	 protected void hook336( File envHome) throws DatabaseException { envLogger=initLogger(envHome); hook336__wrappee__BBase(envHome); }

	 protected void hook337__wrappee__BBase() throws DatabaseException { }

	 protected void hook337() throws DatabaseException { closeLogger(); hook337__wrappee__BBase(); }

	 private Logger envLogger;

	 private Logger initLogger( File envHome) throws DatabaseException { return new EnvironmentImpl_initLogger(this,envHome).execute(); }

	 public void closeLogger(){ Handler[] handlers=envLogger.getHandlers(); for (int i=0; i < handlers.length; i++) { handlers[i].close(); } }

	 public Logger getLogger(){ return envLogger; }

	 @MethodObject static  class  EnvironmentImpl_initLogger {
		 EnvironmentImpl_initLogger( EnvironmentImpl _this, File envHome){ this._this=_this; this.envHome=envHome; }

		 Logger execute() throws DatabaseException { logger=Logger.getAnonymousLogger(); logger.setUseParentHandlers(false); level=Tracer.parseLevel(_this,EnvironmentParams.JE_LOGGING_LEVEL); logger.setLevel(level); return logger; }
