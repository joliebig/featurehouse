...
public class SimplePrintVisitor extends AbstractFSTPrintVisitor  {
	...
	public boolean visit(FSTNonTerminal nonTerminal) {
		...
		if (nonTerminal.getType().equals("IfDefTL")) {
			printFeatures(nonTerminal,true);
			{
				FSTNode v=getChild(nonTerminal, "PPIfDef_TopLevel");
				if (v!=null) {
					v.accept(this);
				}
			}
		...
		if (nonTerminal.getType().equals("StructDec")) {
			...
			printToken("{");
			{
				FSTNode v=getChild(nonTerminal, "Sequence_CodeUnit_TypeLevel");
				if (v!=null) {
					v.accept(this);
				}
			}
		...
		if (nonTerminal.getType().equals("IfDefYL")) {
			printFeatures(nonTerminal,true);
			{
				FSTNode v=getChild(nonTerminal, "PPIfDef_TypeLevel");
				if (v!=null) {
					v.accept(this);
				}
			}
		...
		if (nonTerminal.getType().equals("Sequence_CodeUnit_TypeLevel")) {
			printFeatures(nonTerminal,true);
			for (FSTNode v : getChildren(nonTerminal,"CodeUnit_TypeLevel")) {
				v.accept(this);
			}
			printFeatures(nonTerminal,false);
			return false;
		}
		if (nonTerminal.getType().equals("PPIfDef_TopLevel")) {
			printFeatures(nonTerminal,true);
			{
				FSTNode v=getChild(nonTerminal, "IfDefLine");
				if (v!=null) {
					v.accept(this);
				}
			}
			...
			for (FSTNode v : getChildren(nonTerminal,"IfElseIf_TopLevel")) {
				v.accept(this);
			}
			{
				FSTNode v=getChild(nonTerminal, "PPIfDef_TopLevel_Else");
				if (v!=null) {
					v.accept(this);
				}
			}
		...
		if (nonTerminal.getType().equals("PPIfDef_TopLevel_Else")) {
			printFeatures(nonTerminal,true);
			hintDecIndent();
			printToken("#");
			printToken("else");
		...
		if (nonTerminal.getType().equals("PPIfDef_TypeLevel")) {
			printFeatures(nonTerminal,true);
			{
				FSTNode v=getChild(nonTerminal, "IfDefLine");
				if (v!=null) {
			...
			for (FSTNode v : getChildren(nonTerminal,"IfElseIf_TypeLevel")) {
				v.accept(this);
			}
			{
				FSTNode v=getChild(nonTerminal, "IfElseIf_TypeLevel_Else");
				if (v!=null) {
					v.accept(this);
				}
			}
		...
		if (nonTerminal.getType().equals("IfElseIf_BlockLevel_Else")) {
			printFeatures(nonTerminal,true);
			hintDecIndent();
			printToken("#");
			printToken("else");
			...
			hintNewLine();
			{
				FSTNode v=getChild(nonTerminal, "Sequence_CodeUnit_InBlock");
				if (v!=null) {
					v.accept(this);
				}
			}
		...
		if (nonTerminal.getType().equals("IfElseIf_TypeLevel_Else")) {
			printFeatures(nonTerminal,true);
			hintDecIndent();
			printToken("#");
			printToken("else");
	...
	protected boolean isSubtype(String type, String expectedType) {
		...
		if (type.equals("StmtYL") && expectedType.equals("CodeUnit_TypeLevel")) return true;
		...
		if (type.equals("IfDefYL") && expectedType.equals("CodeUnit_TypeLevel")) return true;
