...
public class SimplePrintVisitor extends AbstractFSTPrintVisitor  {
	...
	public boolean visit(FSTNonTerminal nonTerminal) {
		...
		if (nonTerminal.getType().equals("ClassDeclaration")) {
			{
				FSTNode v=getChild(nonTerminal, "Modifiers");
				if (v!=null) {v=handleJMLModifier(v);
					v.accept(this);
				}
			}
			{
	...
	private FSTNode handleJMLModifier(FSTNode v) {
		System.out.println("hm "+((FSTTerminal)v).getBody());
		...
		t.setBody(t.getBody().replaceAll(mod, "/*@"+mod+"@*/"));
		}
		return t;
