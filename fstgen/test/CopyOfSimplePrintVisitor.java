...
public class CopyOfSimplePrintVisitor extends AbstractFSTPrintVisitor  {
	public CopyOfSimplePrintVisitor(PrintStream out) {
		super(out); generateSpaces=true;
	}
	public CopyOfSimplePrintVisitor() {
	...
	public boolean visit(FSTNonTerminal nonTerminal) {
		...
		if (nonTerminal.getType().equals("MethodSpecification1")) {
			printFeatures(nonTerminal,true);
			{
		...
		if (nonTerminal.getType().equals("ExtendingSpec")) {
			printFeatures(nonTerminal,true);
			{
				FSTNode v=getChild(nonTerminal, "Specification");
				if (v!=null) {
		...
		if (nonTerminal.getType().equals("Specification1")) {
			printFeatures(nonTerminal,true);
			{
		...
		if (nonTerminal.getType().equals("Specification2")) {
			printFeatures(nonTerminal,true);
			{
				FSTNode v=getChild(nonTerminal, "RedundantSpec");
				if (v!=null) {
	...
	private boolean isJMLModifier(FSTNode v) {
		...
		FSTTerminal t = (FSTTerminal)v;
        for(String mod:JMLModifiers)
		if(t.getBody().equals(mod))
			return true;
        System.out.println("false:"+t.getBody());
		return false;
	}
	protected boolean isSubtype(String type, String expectedType) {
		...
		if (type.equals("MethodDeclarationWithoutSpec") && expectedType.equals("ClassOrInterfaceBodyDeclaration")) return true;
		...
		if (type.equals("MethodSpecification1") && expectedType.equals("MethodSpecification")) return true;
		...
		if (type.equals("ExtendingSpec") && expectedType.equals("MethodSpecification")) return true;
		...
		if (type.equals("ConstructorDeclarationWithoutSpec") && expectedType.equals("ClassOrInterfaceBodyDeclaration")) return true;
		...
		if (type.equals("Specification1") && expectedType.equals("Specification")) return true;
		...
		if (type.equals("Specification2") && expectedType.equals("Specification")) return true;
