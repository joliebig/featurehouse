...
public class LexGen extends JavaCCGlobals implements JavaCCParserConstants {
	...
	static void PrintClassHead() {
		...
		try {
			File tmp = new File(Options.getOutputDirectory(), tokMgrClassName
					+ ".java");
			ostr = new java.io.PrintWriter(new java.io.BufferedWriter(
					new java.io.FileWriter(tmp), 8092));
				...
				if (kind == PACKAGE || kind == IMPORT) {
					for (; i < cu_to_insertion_point_1.size(); i++) {
						...
						if (kind == SEMICOLON || kind == ABSTRACT
								|| kind == FINAL || kind == PUBLIC
								|| kind == CLASS || kind == INTERFACE) {
							cline = ((Token) (cu_to_insertion_point_1
									.elementAt(l))).beginLine;
							ccol = ((Token) (cu_to_insertion_point_1
									.elementAt(l))).beginColumn;
							for (j = l; j < i; j++) {
								printToken((Token) (cu_to_insertion_point_1
										.elementAt(j)), ostr);
							}
							if (kind == SEMICOLON)
								printToken((Token) (cu_to_insertion_point_1
			...
			ostr.println("import cide.gparser.*;");
			ostr.println("public class " + tokMgrClassName + " implements "
					+ cu_name + "Constants");
			ostr.println("{"); // }
		} catch (java.io.IOException err) {
			JavaCCErrors.semantic_error("Could not create file : "
		...
		if (token_mgr_decls != null && token_mgr_decls.size() > 0) {
			...
			for (j = 0; j < token_mgr_decls.size(); j++) {
				...
				if (t.kind == IDENTIFIER && commonTokenActionNeeded
			...
			if (commonTokenActionNeeded && !commonTokenActionSeen)
				JavaCCErrors
						.warning("You have the COMMON_TOKEN_ACTION option set. But it appears you have not defined the method :\n"
								+ "      "
								+ staticString
			...
			JavaCCErrors
					.warning("You have the COMMON_TOKEN_ACTION option set. But you have not defined the method :\n"
							+ "      "
							+ staticString
							+ "void CommonTokenAction(Token t)\n"
							+ "in your TOKEN_MGR_DECLS. The generated token manager will not compile.");
		...
		ostr.println("  public " + staticString
	...
	static void DumpDebugMethods() {
		...
		ostr.println("  protected " + staticString
		...
		ostr
				.println("  protected "
						+ staticString
						+ " final String jjKindsForStateVector(int lexState, int[] vec, int start, int end)");
		ostr.println("  {");
		...
		ostr
				.println("     int[] stateSet = statesForState[curLexState][vec[i]];");
		...
		ostr.println("       {");
		ostr
				.println("          kindDone[kindForState[lexState][state]] = true;");
		ostr.println("          if (cnt++ > 0)");
		ostr.println("             retVal += \", \";");
		ostr.println("          if (cnt % 5 == 0)");
		ostr.println("             retVal += \"\\n     \";");
		ostr
				.println("          retVal += tokenImage[kindForState[lexState][state]];");
		ostr.println("       }");
		ostr.println("     }");
		ostr.println("    }");
		ostr.println("    if (cnt == 0)");
	...
	public static void start() {
		...
		while (e.hasMoreElements()) {
			...
			for (i = 0; i < allTps.size(); i++) {
				...
				for (j = 0; j < rexps.size(); j++) {
					...
					if (curRE instanceof RStringLiteral
						...
						((RStringLiteral) curRE).GenerateDfa(ostr,
								curRE.ordinal);
						if (i != 0 && !mixed[lexStateIndex]
								&& ignoring != ignore)
					...
					if (respec.nextState != null
							&& !respec.nextState
									.equals(lexStateName[lexStateIndex]))
			...
			for (i = 0; i < initialState.epsilonMoves.size(); i++)
				((NfaState) initialState.epsilonMoves.elementAt(i))
						.GenerateCode();
			...
			if (initialState.kind != Integer.MAX_VALUE
					&& initialState.kind != 0) {
		...
		if (hasLoop) {
			ostr.println(staticString + "int[] jjemptyLineNo = new int["
					+ maxLexStates + "];");
			ostr.println(staticString + "int[] jjemptyColNo = new int["
					+ maxLexStates + "];");
			ostr.println(staticString + "boolean[] jjbeenHere = new boolean["
					+ maxLexStates + "];");
		}
	...
	static void DumpStaticVarDeclarations() {
		...
		ostr.println(staticString + "protected " + charStreamName
				+ " input_stream;");
		...
		if (Options.getTokenManagerUsesParser() && !Options.getStatic()) {
			ostr.println("public " + tokMgrClassName + "(" + cu_name
					+ " parserArg, " + charStreamName + " stream){");
			ostr.println("   parser = parserArg;");
		} else {
			ostr.println("public " + tokMgrClassName + "(" + charStreamName
					+ " stream){");
		}
		...
		if (Options.getStatic() && !Options.getUserCharStream()) {
			...
			ostr
					.println("      throw new TokenMgrError(\"ERROR: Second call to constructor of static lexer. You must use ReInit() to initialize the static variables.\", TokenMgrError.STATIC_LEXER_ERROR);");
			...
			ostr
					.println("      throw new Error(\"ERROR: Cannot use a static CharStream class with a non-static lexical analyzer.\");");
		}
		...
		ostr.println(staticString + "public void ReInit(" + charStreamName
				+ " stream)");
		ostr.println("{");
		...
		ostr.println("{");
		ostr.println("   int i;");
		ostr.println("   jjround = 0x"
				+ Integer.toHexString(Integer.MIN_VALUE + 1) + ";");
		ostr.println("   for (i = " + stateSetSize + "; i-- > 0;)");
		ostr.println("      jjrounds[i] = 0x"
		...
		ostr.println("{");
		ostr.println("   if (lexState >= " + lexStateName.length
				+ " || lexState < 0)");
		ostr
				.println("      throw new TokenMgrError(\"Error: Ignoring invalid lexical state : \""
	...
	static void DumpFillToken() {
		...
		if (hasEmptyMatch) {
			...
			if (keepLineCol) {
				ostr
						.println("      t.beginLine = t.endLine = input_stream.getBeginLine();");
				ostr
			...
			ostr.println("   {");
			ostr
					.println("      String im = jjstrLiteralImages[jjmatchedKind];");
			ostr
					.println("      t.image = (im == null) ? input_stream.GetImage() : im;");

			if (keepLineCol) {
				ostr
						.println("      t.beginLine = input_stream.getBeginLine();");
				ostr
						.println("      t.beginColumn = input_stream.getBeginColumn();");
				ostr.println("      t.endLine = input_stream.getEndLine();");
				ostr
						.println("      t.endColumn = input_stream.getEndColumn();");
			}
	...
	static void DumpGetNextToken() {
		...
		if (Options.getDebugTokenManager())
			ostr
					.println("      debugStream.println(\"Returning the <EOF> token.\");");
		...
		for (i = 0; i < maxLexStates; i++) {
				...
				if (singlesToSkip[i].asciiMoves[0] != 0L
					...
					ostr
							.println(prefix
									...
									+ " && (0x"
									+ Long
											.toHexString(singlesToSkip[i].asciiMoves[1])
									+ "L & (1L << (curChar & 077))) != 0L)");
				}
			...
			if (initMatch[i] != Integer.MAX_VALUE && initMatch[i] != 0) {
				if (Options.getDebugTokenManager())
					ostr
							.println("      debugStream.println(\"   Matched the empty string as \" + tokenImage["
									+ initMatch[i] + "] + \" token.\");");
				...
				ostr.println(prefix + "jjmatchedKind = 0x"
			...
			if (canMatchAnyChar[i] != -1) {
				if (initMatch[i] != Integer.MAX_VALUE && initMatch[i] != 0)
					...
					ostr.println(prefix
							+ "if (jjmatchedPos == 0 && jjmatchedKind > "
							+ canMatchAnyChar[i] + ")");
				ostr.println(prefix + "{");

				if (Options.getDebugTokenManager())
					ostr
							.println("           debugStream.println(\"   Current character matched as a \" + tokenImage["
		...
		if (maxLexStates > 0) {
			ostr.println(prefix + "   if (jjmatchedKind != 0x"
					+ Integer.toHexString(Integer.MAX_VALUE) + ")");
			ostr.println(prefix + "   {");
			...
			if (Options.getDebugTokenManager()) {
				if (Options.getJavaUnicodeEscape()
					...
					ostr
							.println("    debugStream.println(\"****** FOUND A \" + tokenImage[jjmatchedKind] + \" MATCH (\" + TokenMgrError.addEscapes(new String(input_stream.GetSuffix(jjmatchedPos + 1))) + \") ******\\n\");");
				else
					ostr
							.println("    debugStream.println(\"****** FOUND A \" + tokenImage[jjmatchedKind] + \" MATCH (\" + TokenMgrError.addEscapes(new String(input_stream.GetSuffix(jjmatchedPos + 1))) + \") ******\\n\");");
			}
			...
			if (hasSkip || hasMore || hasSpecial) {
				...
				if (hasSkip || hasSpecial) {
					...
					if (maxLexStates > 1) {
						ostr
								.println("         if (jjnewLexState[jjmatchedKind] != -1)");
						ostr
								.println(prefix
										+ "         curLexState = jjnewLexState[jjmatchedKind];");
					}
		...
		Outer: for (int i = 0; i < maxOrdinal; i++) {
			...
			for (;;) {
				if (((act = (Action) actions[i]) == null
						|| act.action_tokens == null || act.action_tokens
						.size() == 0)
						&& !canLoop[lexStates[i]])
					continue Outer;
				...
				if (initMatch[lexStates[i]] == i && canLoop[lexStates[i]]) {
					...
					ostr.println("            if (jjbeenHere[" + lexStates[i]
							+ "] &&");
					ostr.println("                jjemptyLineNo["
							+ lexStates[i]
							+ "] == input_stream.getBeginLine() && ");
					ostr.println("                jjemptyColNo[" + lexStates[i]
							+ "] == input_stream.getBeginColumn())");
					ostr
							.println("               throw new TokenMgrError((\"Error: Bailing out of infinite loop caused by repeated empty string matches at line \" + input_stream.getBeginLine() + \", column \" + input_stream.getBeginColumn() + \".\"), TokenMgrError.LOOP_DETECTED);");
					ostr.println("            jjemptyLineNo[" + lexStates[i]
							+ "] = input_stream.getBeginLine();");
					ostr.println("            jjemptyColNo[" + lexStates[i]
							+ "] = input_stream.getBeginColumn();");
					ostr.println("            jjbeenHere[" + lexStates[i]
							+ "] = true;");
					ostr.println("         }");
				}
				...
				if (RStringLiteral.allImages[i] != null)
					ostr.println("(jjstrLiteralImages[" + i + "]);");
						...
						|| Options.getUserCharStream())
					ostr
							.println("(input_stream.GetSuffix(jjimageLen + (lengthOfMatch = jjmatchedPos + 1)));");
				else
					ostr
							.println("(input_stream.GetSuffix(jjimageLen + (lengthOfMatch = jjmatchedPos + 1)));");
	...
	public static void DumpTokenActions() {
		...
		Outer: for (i = 0; i < maxOrdinal; i++) {
			...
			for (;;) {
				...
				if (i == 0) {
					ostr.println("      image = null;"); // For EOF no image
															// is there
				} else {
					ostr.println("        if (image == null)");
					ostr.println("            image = new StringBuffer();");
