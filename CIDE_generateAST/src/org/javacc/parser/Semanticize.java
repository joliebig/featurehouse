...
public class Semanticize extends JavaCCGlobals {
  ...
  static public void start() throws MetaParseException {
     ...
     */
    for (java.util.Enumeration _enum = bnfproductions.elements(); _enum.hasMoreElements();) {
      ExpansionTreeWalker.postOrderWalk(((NormalProduction)_enum.nextElement()).expansion, new LookaheadFixer());
    }
     ...
     */
    for (java.util.Enumeration _enum = bnfproductions.elements(); _enum.hasMoreElements();) {
      NormalProduction p = (NormalProduction)_enum.nextElement();
     ...
     */
    for (java.util.Enumeration _enum = bnfproductions.elements(); _enum.hasMoreElements();) {
      ExpansionTreeWalker.preOrderWalk(((NormalProduction)_enum.nextElement()).expansion, new ProductionDefinedChecker());
    }
     ...
     */
    for (java.util.Enumeration _enum = rexprlist.elements(); _enum.hasMoreElements();) {
      TokenProduction tp = (TokenProduction)(_enum.nextElement());
    ...
    while (emptyUpdate) {
      emptyUpdate = false;
      for (java.util.Enumeration _enum = bnfproductions.elements(); _enum.hasMoreElements();) {
        NormalProduction prod = (NormalProduction)_enum.nextElement();
    ...
    if (Options.getSanityCheck() && JavaCCErrors.get_error_count() == 0) {
      ...
      for (java.util.Enumeration _enum = bnfproductions.elements(); _enum.hasMoreElements();) {
        ExpansionTreeWalker.preOrderWalk(((NormalProduction)_enum.nextElement()).expansion, new EmptyChecker());
      }
      ...
      if (JavaCCErrors.get_error_count() == 0) {
        for (java.util.Enumeration _enum = bnfproductions.elements(); _enum.hasMoreElements();) {
          ExpansionTreeWalker.preOrderWalk(((NormalProduction)_enum.nextElement()).expansion, new LookaheadChecker());
        }
      }
  ...
  public static boolean hasIgnoreCase(java.util.Hashtable table, String str) {
    ...
    for (java.util.Enumeration _enum = table.elements(); _enum.hasMoreElements();) {
      rexp = (RegularExpression)(_enum.nextElement());
  ...
  public static boolean emptyExpansionExists(Expansion exp) {
    if (exp instanceof NonTerminal) {
      ...
      for (java.util.Enumeration _enum = ((Choice)exp).choices.elements(); _enum.hasMoreElements();) {
        if (emptyExpansionExists((Expansion)_enum.nextElement())) {
          return true;
        }
      }
      return false;
    } else if (exp instanceof Sequence) {
      for (java.util.Enumeration _enum = ((Sequence)exp).units.elements(); _enum.hasMoreElements();) {
        if (!emptyExpansionExists((Expansion)_enum.nextElement())) {
          return false;
        }
      }
  ...
  static private void addLeftMost(NormalProduction prod, Expansion exp) {
	...
	prod.leftExpansions = newle;
      ...
      for (java.util.Enumeration _enum = ((Choice)exp).choices.elements(); _enum.hasMoreElements();) {
        addLeftMost(prod, (Expansion)_enum.nextElement());
      }
    } else if (exp instanceof Sequence) {
      for (java.util.Enumeration _enum = ((Sequence)exp).units.elements(); _enum.hasMoreElements();) {
        Expansion e = (Expansion)_enum.nextElement();
        addLeftMost(prod, e);
  ...
  static private boolean rexpWalk(RegularExpression rexp) {
    if (rexp instanceof RJustName) {
      ...
      for (java.util.Enumeration _enum = ((RChoice)rexp).choices.elements(); _enum.hasMoreElements();) {
        if (rexpWalk((RegularExpression)_enum.nextElement())) {
          return true;
        }
      }
      return false;
    } else if (rexp instanceof RSequence) {
      for (java.util.Enumeration _enum = ((RSequence)rexp).units.elements(); _enum.hasMoreElements();) {
        if (rexpWalk((RegularExpression)_enum.nextElement())) {
          return true;
        }
      }
