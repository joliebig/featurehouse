...
def log2(n, log=math.log, c=math.log(2)):
 ...
 def is_executable(prog):
    ...
    return (info.st_uid == os.getuid() and (info.st_mode & 64) or
            info.st_gid == os.getgid() and (info.st_mode & 8) or
            info.st_mode & 1)
...
_cachefile = options["Tokenizer", "crack_image_cache"] crack_images = ImageStripper(_cachefile).analyze
