...
HEADER_NAME = r"[\w\.\-\*]+" HEADER_VALUE = r".+" INTEGER = r"[\d]+" REAL = r"[\d]+[\.]?[\d]*" BOOLEAN = (False, True) SERVER = r"([\w\.\-]+(:[\d]+)?)" PORT = r"[\d]+" EMAIL_ADDRESS = r"[\w\-\.]+@[\w\-\.]+" PATH = r"[\w \$\.\-~:\\/\*\@\=]+" VARIABLE_PATH = PATH + r"%" FILE = r"[\S]+" FILE_WITH_PATH = PATH IP_LIST = r"\*|localhost|((\*|[01]?\d\d?|2[0-4]\d|25[0-5])\.(\*|[01]?\d" \
          r"\d?|2[0-4]\d|25[0-5])\.(\*|[01]?\d\d?|2[0-4]\d|25[0-5])\.(\*" \
          r"|[01]?\d\d?|2[0-4]\d|25[0-5]),?)+" IMAP_FOLDER = r"[^,]+" IMAP_ASTRING = [] for _i in range(1, 128):
