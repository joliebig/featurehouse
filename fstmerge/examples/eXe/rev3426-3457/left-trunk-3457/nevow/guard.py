...
class SessionWrapper:
    ...
    def _delegate(self, ctx, segments):
        ...
        if userpass != ('',''):
            self.sessions[httpAuthSessionKey] = self.sessionFactory(self, httpAuthSessionKey)
            sz = self.sessions[httpAuthSessionKey]
            sz.checkExpired()
    ...
    def createSession(self, request, segments):
        ...
        if self.useCookies:
            ...
            request.addCookie(self.cookieKey, newCookie,
                              ...
                              secure=secure)
        self.sessions[newCookie] = self.sessionFactory(self, newCookie)
