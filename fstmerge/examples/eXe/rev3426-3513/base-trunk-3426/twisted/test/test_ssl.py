...
class TLSTestCase(unittest.TestCase):
    ...
    def _runTest(self, clientProto, serverProto, clientIsServer=False):
        ...
        if clientIsServer:
            ...
            inCharge = sf
        inCharge.done = 0
        self.port = reactor.listenTCP(0, sf, interface="127.0.0.1")
        port = self.port
...
class BufferingTestCase(unittest.TestCase):
    ...
    def testOpenSSLBuffering(self):
        self.serverProto = SingleLineServerProtocol()
        serverProto = self.serverProto
        self.clientProto = RecordingClientProtocol()
        clientProto = self.clientProto
        server = protocol.ServerFactory()
        self.client = protocol.ClientFactory()
        client = self.client
        ...
        self.port = reactor.listenSSL(0, server, sCTX, interface='127.0.0.1')
        port = self.port
