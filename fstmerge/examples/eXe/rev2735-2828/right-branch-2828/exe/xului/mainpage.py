...
class MainPage(RenderableLivePage):
    ...
    def handleTinyMCEimageChoice(self, client, tinyMCEwin, tinyMCEwin_name, tinyMCEfield, local_filename, preview_filename):
        """
        Once an image is selected in the file browser that is spawned by the TinyMCE image dialog,
        copy this file (which is local to the user's machine) into the server space, under a preview directory
        (after checking if this exists, and creating it if necessary).
        Note that this IS a "cheat", in violation of the client-server separation, but can be done
        since we know that the eXe server is actually sitting on the client host.
        """
        server_filename = ""
        callback_errors = ""
        errors = 0
        ...
        if not previewDir.exists():
            ...
            errors += 1
        if errors == 0:
            log.debug("checking fo the file...")
            localImagePath = Path(local_filename)
            if not localImagePath.exists() or not localImagePath.isfile():
                client.alert(_(u'Image file %s is not found, cannot preview it') % localImagePath)
                ...
                errors += 1
        if errors == 0:
            log.debug("image previews directory AND the requested image file exists; proceeding with the copy...")
        log.debug("r3m0 DEBUG: be sure to have already copied " + preview_filename + " into the webdir's previews directory!")
        try:
            server_filename = previewDir.joinpath(preview_filename);
            log.debug("handleTinyMCEimageChoice about to copy image from \'" + local_filename + "\' to \'" + server_filename.abspath().encode('utf-8') + "\'.");
            shutil.copyfile(local_filename, server_filename.abspath().encode('utf-8'));
        except Exception, e:
            client.alert(_('SAVE FAILED!\n%s' % str(e)))
