...
class FieldWithResources(Field):
    ...
    A Generic iDevice is built up of these fields.  Each field can be
    rendered as an XHTML element
    Used by TextAreaField and ClozeField to encapsulate all the multi-resource handling
    which can now be included via the tinyMCE RichTextArea
    NOTE: Nope, nothing special here as far as the field goes, but (silly me) added this
    layer in here to correspond to the ElementWithResources level in the element tree
    """
    def __init__(self, name, instruc="", content=""):
        """
        Initialize 
...
class QuizOptionField(Field):
    ...
    def __init__(self, question, idevice, name="", instruc=""):
        ...
        self.answer    = TextAreaField(x_(u'Option'), idevice._answerInstruc, x_(u''))
        self.answer.idevice = idevice
        self.feedback = TextAreaField(x_(u'Feedback'), idevice._feedbackInstruc, x_(u''))
        self.feedback.idevice = idevice
class QuizQuestionField(Field):
    ...
    def __init__(self, idevice, name, instruc=""):
        ...
        self.options              = []
        self.idevice              = idevice
        self.question             = TextAreaField(x_(u'Question'), idevice._questionInstruc, x_(u''))
        self.question.idevice     = idevice
        self.hint                 = TextAreaField(x_(u'Hint'), idevice._hintInstruc, x_(u''))
        self.hint.idevice         = idevice
    def addOption(self):
        """
...
class SelectOptionField(Field):
    ...
    def __init__(self, question, idevice, name="", instruc=""):
        ...
        self.question  = question
        self.idevice = idevice
        self.answer    = TextAreaField(x_(u'Options'), question._optionInstruc, x_(u''))
        self.answer.idevice = idevice
...
can tend to confuse learners.""")
        self.question = TextAreaField(x_(u'Question:'), self.questionInstruc, x_(u''))
        self.question.idevice = idevice
        self.options              = []
...
to provide the learner with.""")
        self.feedback = TextAreaField(x_(u'Feedback:'), self.feedbackInstruc, x_(u''))
        self.feedback.idevice = idevice
