...
class OldConnectedUDPTestCase(PortCleanerUpper):
    def testStartStop(self):
        client = Client()
        client.startedDeferred = defer.Deferred()
        d = client.startedDeferred
    ...
    def testSendPackets(self):
        server = Server()
        server.startedDeferred = defer.Deferred()
        serverStarted = server.startedDeferred
        client = Client()
        client.startedDeferred = defer.Deferred()
        clientStarted = client.startedDeferred
        ...
        def cbClientStarted(ignored):
            server.packetReceived = defer.Deferred()
            clientSend = server.packetReceived
            client.packetReceived = defer.Deferred()
            serverSend = client.packetReceived
...
class UDPTestCase(unittest.TestCase):
    ...
    def testSendPackets(self):
        ...
        def cbClientStarted(ignored):
            ...
            def cbClientSend(ignored):
                if clientWrites:
                    server.packetReceived = defer.Deferred()
                    nextClientWrite = server.packetReceived
    ...
    def testConnectionRefused(self):
        ...
        def cbStarted(ignored):
            client.startedDeferred = defer.Deferred()
            connectionRefused = client.startedDeferred
