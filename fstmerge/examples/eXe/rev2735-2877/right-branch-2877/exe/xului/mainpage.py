...
class MainPage(RenderableLivePage):
    ...
    def get_printdir_relative2web(self, exported_dir):
        ...
        get_printdir_relative2webdir() will merely strip off the initial 
        portions of the absolute pathname which precede the webdir, such 
        that the calling javascript may give an http url relative to the 
        server path.
        """
        web_dirname = self.config.webDir.abspath().encode('utf-8')
        http_relative_pathname = "http://127.0.0.1:"+str(self.config.port) \
                                   +exported_dir.replace(web_dirname, "", 1)
