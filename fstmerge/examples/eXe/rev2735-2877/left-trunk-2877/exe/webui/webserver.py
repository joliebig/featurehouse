...
class WebServer:
    ...
    def find_port(self):
        """
        Previously part of the run() method, this will find the port for this server.
        Moved outside such that it could be called prior to run() [via application's serve()], 
        which doesn't get called until the client has already been started [via application's launch()]. 
        Instead, we want to determine the server's port first [via application's prelaunch()] such that the client knows it.
        Note: for safety down the road once run() is called, the port will be set to -1 if none is found.
        """
        port_test_done = 0
        found_port = 0
        found_other_eXe = 0
        ...
        while not port_test_done:
            ...
            try:
                log.debug("find_port(): trying to listenTCP on port# %d", test_port_num)
                reactor.listenTCP(test_port_num, appserver.NevowSite(self.root),
                                  interface="127.0.0.1")
                log.debug("find_port(): still here without exception after listenTCP on port# %d", test_port_num)
                found_port = 1
                port_test_done = 1
            except CannotListenError, exc:
                log.debug("find_port(): caught exception after listenTCP on port# %d, exception = %s", test_port_num, exc)
                last_exception = exc
                test_this_host = "127.0.0.1"
                log.debug("find_port(): appears that a service is already running on port# %d, seeing if it is another eXe", test_port_num)
                s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s.connect((test_this_host, test_port_num))
                s.setblocking(0)
                s.settimeout(3)                # and send a basic HTTP request to obtain the server info:
                s.send('GET / HTTP/1.0\r\n\r\n')
                try:
                    data = s.recv(1024)
        ...
        if found_port:
            ...
            self.config.port = -1
            if found_other_eXe:
                log.error("find_port(): found another eXe server running on port# %d; only one eXe server allowed to run at a time", test_port_num)
            else:
                log.error("find_port(): Can't listen on interface 127.0.0.1, ports %s-%s, last exception: %s" %
