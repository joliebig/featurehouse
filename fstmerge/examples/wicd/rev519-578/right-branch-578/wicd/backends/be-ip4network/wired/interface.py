...
def needscurrentprofile(method):
    ...
    def profilecheck(self, *args, **kwargs):
        if not hasattr(self.interface, 'current_profile'):
            raise WicdError('Must run set_current_profile() before %s()' % method.func_name)
...
class BackendWiredInterface(BaseInterface):
    ...
    def __init__(self, interface_name, status_change_callback):
        ...
        self._recommended_profile = None
    @needsidle
    def do_update(self):
    ...
    @needsidle
    def do_delete_profile(self, profile_name):
        ''' Deletes the network profile with the specified name. '''
        if hasattr(self.interface, 'current_profile'):
            if self.interface.current_profile is \
               self.interface._get_profile('profile_name', profile_name):
                raise WicdError('Cannot delete active profile.')
        self.interface.delete_profile(profile_name)
    ...
    def set_recommended_profile(self, profile_name):
        profile = self.interface._get_profile('profile_name', profile_name)
        self._recommended_profile = profile
    def get_has_profile_property(self, profile_name, name):
        profile = self.interface._get_profile('profile_name', profile_name)
        return name in profile
    def get_profile_property(self, profile_name, name):
        profile = self.interface._get_profile('profile_name', profile_name)
        if not name in profile:
            raise WicdError('Current network has no property %s' % name)
        return profile[name]
    @needsidle
    def do_connect(self, profile_name):
        ...
        profile = self.interface._get_profile('profile_name', profile_name)
        self.interface.connect(profile, finished_callback)
    ...
    def do_autoconnect(self, value):
        ...
        if self._recommended_profile:
            self.do_connect(self._recommended_profile['profile_name'])
