    ...
    def perform(self):
        ...
        for interface in interfaces:
            ...
            try:
                ...
                logging.exception('error occured while running %s: %s',
                              self.method_name,
                              e)
            else:
        ...
        for name, value in self.items(location):
            if name.startswith(item_base):
                try:
                    ...
                    logging.error( '%s is not a valid item', name )
        return items
    def find_location(self):
        for location in self.locations:
            results = location.run_tests()
            logging.debug('%s results: %s', location, results)
