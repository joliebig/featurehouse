...
class InvalidIPException(WicdError): pass
...
def smart_type(value):
    ...
    return value
def validate_ips(dict_, names, optional=False):
    for name in names:
        value = dict_.get(name, None)
        if not optional:
            if value is None:
                raise InvalidIPException('%s: must specify IP address' % name)
            else:
                if not bool(is_valid_ip(value)):
            ...
            if not value is None:
                if not bool(is_valid_ip(value)):
                    raise InvalidIPException('%s is not a valid IP address' % value)
