...
if options.ping:
    ...
    try:
        while True:
            ...
            print 'Reponse time: %s' % delta
            times.append(delta)
            time.sleep(0.1)
    except KeyboardInterrupt:
        raise
    finally:
        print 'min: %s max: %s avg: %s' % (min(times), max(times), avg(times))
elif options.listen_for_signal:
    def signal_change(*args):
        print 'Signal %s: ' % options.name, args
...
if options.load_configuration:
    ...
    print _has_data(dbus_ifaces['interface'].GetInterfaceData(options.interface_name,
                                                       options.get_data,
                                                       options.data))
