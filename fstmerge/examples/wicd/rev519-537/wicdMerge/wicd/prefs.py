...
def alert(parent, message):
    ...
    dialog = gtk.MessageDialog(parent, gtk.DIALOG_MODAL, gtk.MESSAGE_WARNING,                               gtk.BUTTONS_OK)
...
class  PreferencesDialog (object) :
	...
	def build_preferences_diag(self):
        ...
        self.wiredcheckbox = setup_label("pref_always_check",                                         'wired_always_on')
        self.wiredcheckbox.set_active(daemon.GetAlwaysShowWiredInterface())
        self.reconnectcheckbox = setup_label("pref_auto_check",                                             'auto_reconnect')
        ...
        self.displaytypecheckbox = setup_label("pref_dbm_check",                                               'display_type_dialog')
        self.displaytypecheckbox.set_active(daemon.GetSignalDisplayType())
        self.usedefaultradiobutton = setup_label("pref_use_def_radio",                                                 'use_default_profile')
        self.showlistradiobutton = setup_label("pref_prompt_radio",                                               'show_wired_list')
        self.lastusedradiobutton = setup_label("pref_use_last_radio",                                               'use_last_used_profile')
        ...
        dhcp_list = [self.dhcpautoradio, self.dhclientradio, self.dhcpcdradio,                     self.pumpradio]
        ...
        wired_link_list = [self.linkautoradio, self.ethtoolradio,                           self.miitoolradio]
        wired_link_method = daemon.GetLinkDetectionTool()
        wired_link_list[wired_link_method].set_active(True)
        self.flushautoradio = setup_label("flush_auto_radio",                                          'wicd_auto_config')
        self.ipflushradio = setup_label("ip_flush_radio")
        self.routeflushradio = setup_label("route_flush_radio")
        flush_list = [self.flushautoradio, self.ipflushradio,                      self.routeflushradio]
        ...
        self.wpadrivers = ["wext", "hostap", "madwifi", "atmel", "ndiswrapper",                           "ipw", "ralink legacy"]
        found = False
        ...
        self.useGlobalDNSCheckbox = setup_label("pref_global_check",                                                'use_global_dns')
        ...
        self.useGlobalDNSCheckbox.connect("toggled", checkboxTextboxToggle,                                          (self.dns1Entry, self.dns2Entry,                                           self.dns3Entry))
	...
	def save_results(self):
        ...
        daemon.SetGlobalDNS(self.dns1Entry.get_text(), self.dns2Entry.get_text(),                            self.dns3Entry.get_text())
