...
class  ConnectThread (threading.Thread) :
    ...
    """
	    ...
	    def __init__(self, network, wireless, wired, before_script, after_script,                 disconnect_script, gdns1, gdns2, gdns3, wiface, liface, debug):
	...
	def set_dns_addresses(self):
        ...
        if self.network.get('use_global_dns'):
            BACKEND.SetDNS(misc.Noneify(self.global_dns_1),                             misc.Noneify(self.global_dns_2),                             misc.Noneify(self.global_dns_3))
        elif self.network.get('use_static_dns') and (self.network.get('dns1') or                    self.network.get('dns2') or self.network.get('dns3')):
            self.SetStatus('setting_static_dns')
            BACKEND.SetDNS(self.network.get('dns1'),                                 self.network.get('dns2'),                                 self.network.get('dns3'))
...
class  Wireless (Controller) :
	...
	def get_wpa_driver(self): return self._wpa_driver
        ...
        if self._backend:
            self.wiface = self._backend.WirelessInterface(self.wireless_interface,                                                    self.debug, self.wpa_driver)
	...
	def Connect(self, network, debug=False):
        ...
        self.connecting_thread = WirelessConnectThread(network,            self.wireless_interface, self.wired_interface,            self.wpa_driver, self.before_script, self.after_script,            self.disconnect_script, self.global_dns_1,            self.global_dns_2, self.global_dns_3, self.wiface, self.liface,            debug)
	...
	def CreateAdHocNetwork(self, essid, channel, ip, enctype, key,            enc_used, ics):
        ...
        if ics and ip_parts:
            ...
            misc.Run('iptables -t nat -A POSTROUTING -s ' + net_ip + \
                     ...
                     ' -j MASQUERADE')
            misc.Run('echo 1 > /proc/sys/net/ipv4/ip_forward')
...
class  WirelessConnectThread (ConnectThread) :
    ...
    """
	    def __init__(self, network, wireless, wired, wpa_driver,            before_script, after_script, disconnect_script, gdns1,            gdns2, gdns3, wiface, liface, debug=False):
        ...
        gdns3 -- global DNS server 3
        """
        ConnectThread.__init__(self, network, wireless, wired,            before_script, after_script, disconnect_script, gdns1,            gdns2, gdns3, wiface, liface, debug)
	...
	def run(self):
        ...
        wiface.Associate(self.network['essid'], self.network['channel'],                         self.network['bssid'])
        if self.wpa_driver == 'ralink legacy':
	...
	def generate_psk_and_authenticate(self, wiface):
        ...
        if self.network.get('key'):
            ...
            self.network['psk'] = misc.RunRegex(key_pattern,                                misc.Run(''.join(['wpa_passphrase "',                                         self.network['essid'], '" "',                                         _sanitize(self.network['key']), '"'])))
...
class  Wired (Controller) :
	...
	def get_link_detect(self): return self._link_detect
        ...
        Controller.LoadBackend(self, backend)
        if self._backend:
            self.liface = self._backend.WiredInterface(self.wired_interface,                                                       self.debug)
	...
	def Connect(self, network, debug=False):
        ...
        self.connecting_thread = WiredConnectThread(network,            self.wireless_interface, self.wired_interface,            self.before_script, self.after_script,            self.disconnect_script, self.global_dns_1,            self.global_dns_2, self.global_dns_3, self.wiface, self.liface,            debug)
        self.connecting_thread.setDaemon(True)
        self.connecting_thread.start()
        return True
...
class  WiredConnectThread (ConnectThread) :
    ...
    to the specified network.
    """
	    def __init__(self, network, wireless, wired,            before_script, after_script, disconnect_script, gdns1,            gdns2, gdns3, wiface, liface, debug=False):
        """ Initialise the thread with network information.
        Keyword arguments:
        network -- the network to connect to
        wireless -- name of the wireless interface
