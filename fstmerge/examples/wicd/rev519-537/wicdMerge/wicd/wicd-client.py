...
class  TrayIcon :
	...
	class  TrayConnectionInfo :
		...
		def set_wired_state(self, info):
            ...
            self.tr.set_tooltip(language['connected_to_wired'].replace('$A',                                                                     wired_ip))
		...
		def set_connecting_state(self, info):
            ...
            if info[0] == 'wired' and len(info) == 1:
                ...
                cur_network = info[1]
            self.tr.set_tooltip(language['connecting'] + " to " +                                cur_network + "...")
		...
		def set_not_connected_state(self, info):
            ...
            if wireless.GetKillSwitchEnabled():
                status = (language['not_connected'] + " (" +                         language['killswitch_enabled'] + ")")
            else:
		...
		def get_bandwidth_state(self):
            ...
            if not rcvbytes or not sndbytes:
                return 'idle-'
            activity = self.is_network_active(rcvbytes, self.max_rcv_gain,                                              self.last_rcvbytes)
            receiving = activity[0]
            self.max_rcv_gain = activity[1]
            self.last_rcvbytes = activity[2]
            activity = self.is_network_active(sndbytes, self.max_snd_gain,                                              self.last_sndbytes)
	...
	class  TrayIconGUI (object) :
        ...
        """
            ...
            """
            actions = [                    ('Menu',  None, 'Menu'),                    ('Connect', gtk.STOCK_CONNECT, "Connect"),                    ('About', gtk.STOCK_ABOUT, '_About...', None,                     'About wicd-tray-icon', self.on_about),                    ('Quit',gtk.STOCK_QUIT,'_Quit',None,'Quit wicd-tray-icon',                     self.on_quit),                    ]
		...
		def _add_item_to_menu(self, net_menu, lbl, type_, n_id, is_connecting,                              is_active):
            ...
            if type_ == "__wired__":
                ...
                pb = gtk.gdk.pixbuf_new_from_file_at_size(self._get_img(n_id),                                                          20, 20)
            ...
            if daemon.GetAlwaysShowWiredInterface() or \
               wired.CheckPluggedIn():
                if status == misc.WIRED:
                    ...
                    is_active = False
                self._add_item_to_menu(submenu, "Wired Network", "__wired__", 0,                                       is_connecting, is_active)
            ...
            if num_networks > 0:
                for x in range(0, num_networks):
                    ...
                    if status == misc.WIRELESS and info[1] == essid:
                        ...
                        is_active = False
                    self._add_item_to_menu(submenu, essid, "wifi", x,                                           is_connecting, is_active)
            else:
...
def setup_dbus():
    ...
    try:
        ...
        try:
            ...
            gui.error(None, "Could not connect to wicd's D-Bus interface.  " +                      "Make sure the daemon is started.")
            sys.exit(1)
...
def main(argv):
    ...
    animate = True
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'nha', ['help', 'no-tray',                                                         'no-animate'])
    ...
    bus = dbus_manager.get_bus()
    bus.add_signal_receiver(tray_icon.icon_info.wired_profile_chooser,                            'LaunchChooser', 'org.wicd.daemon')
    bus.add_signal_receiver(tray_icon.icon_info.update_tray_icon,                            'StatusChanged', 'org.wicd.daemon')
    bus.add_signal_receiver(tray_icon.tr.tray_scan_ended, 'SendEndScanSignal',                            'org.wicd.daemon.wireless')
    bus.add_signal_receiver(tray_icon.tr.tray_scan_started,                            'SendStartScanSignal', 'org.wicd.daemon.wireless')
    print 'Done.'
    mainloop = gobject.MainLoop()
    mainloop.run()
...
class  TrayIcon (object) :
	...
	class  TrayIconGUI (object) :
		...
		<<<<<<< /work/joliebig/workspace_sple/featurehouse/fstmerge/fstmerge_tmp1309793774063/fstmerge_var1_3675166832262274044
def on_about(self, data=None):
            """ Opens the About Dialog. """
            dialog = gtk.AboutDialog()
            dialog.set_name('wicd tray icon')
            ...
            dialog.destroy()
=======
>>>>>>> /work/joliebig/workspace_sple/featurehouse/fstmerge/fstmerge_tmp1309793774063/fstmerge_var2_193505822482873496
