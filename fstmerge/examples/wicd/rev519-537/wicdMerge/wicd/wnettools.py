...
def GetWirelessInterfaces():
    ...
    ifnames = []
    ifnames = [iface for iface in os.listdir(dev_dir) if os.path.isdir(dev_dir + iface)               and 'wireless' in os.listdir(dev_dir + iface)]
    return bool(ifnames) and ifnames[0] or None

def GetWiredInterfaces():
    basedir = '/sys/class/net/'
    return [iface for iface in os.listdir(basedir) if not 'wireless'            in os.listdir(basedir + iface) and            open(basedir + iface + "/type").readlines()[0].strip() == "1"]

def NeedsExternalCalls():
    print ("NeedsExternalCalls: returning default of True.  You should " +          "implement this yourself.")
    return True
...
class  BaseInterface (object) :
	...
	def _find_client_path(self, client):
        ...
        """
        paths = ['/sbin/', '/usr/sbin/', '/bin/', '/usr/bin/',                 '/usr/local/sbin/', '/usr/local/bin/']
        for path in paths:
...
class  BaseWirelessInterface (BaseInterface) :
        ...
        for x in lines:
            ...
            if info[2] == ap['essid']:
                if misc.RunRegex(wep_pattern, cell) == 'on':
                    ap['encryption'] = True
                    if info[5] == 'WEP' or (                            (info[5] == 'OPEN' or info[5] == 'SHARED') and                        info[4] == 'WEP'):
	...
	def Authenticate(self, network):
        ...
        if self.wpa_driver == RALINK_DRIVER:
            ...
            cmd = ''.join(['wpa_supplicant -B -i ', self.iface, ' -c "',                       wpath.networks, network['bssid'].replace(':','').lower(),                       '" -D ', self.wpa_driver])
            if self.verbose: print cmd
            misc.Run(cmd)
            ...
            for x in lines:
                ...
                if info[2] == network.get('essid'):
                    if info[5] == 'WEP' or (info[5] == 'OPEN' and \
                        ...
                        cmd = ''.join(['iwconfig ', self.iface, ' key ',                                      network.get('key')])
                        if self.verbose: print cmd
                        misc.Run(cmd)
                    else:
...
class  WirelessInterface (Interface) :
	...
	<<<<<<< /work/joliebig/workspace_sple/featurehouse/fstmerge/fstmerge_tmp1309793772646/fstmerge_var1_6444875202957078335
def _GetRalinkInfo(self):
        """ Get a network info list used for ralink drivers
        Calls iwpriv <wireless interface> get_site_survey, which
        on some ralink cards will return encryption and signal
        ...
        return aps
=======
>>>>>>> /work/joliebig/workspace_sple/featurehouse/fstmerge/fstmerge_tmp1309793772646/fstmerge_var2_7062564756386436660
	...
	<<<<<<< /work/joliebig/workspace_sple/featurehouse/fstmerge/fstmerge_tmp1309793773003/fstmerge_var1_1013897580829916008
def ValidateAuthentication(self, auth_time):
        """ Validate WPA authentication.
            Validate that the wpa_supplicant authentication
            process was successful.
        ...
        return False
=======
>>>>>>> /work/joliebig/workspace_sple/featurehouse/fstmerge/fstmerge_tmp1309793773003/fstmerge_var2_7909088100960048934

	
	<<<<<<< /work/joliebig/workspace_sple/featurehouse/fstmerge/fstmerge_tmp1309793773088/fstmerge_var1_8669985615952085100
def _AuthenticateRalinkLegacy(self, network):
        """ Authenticate with the specified wireless network.
        This function handles Ralink legacy cards that cannot use
        wpa_supplicant.
        ...
        if network.get('key') != None:
            ...
            if network['encryptype'] == "WEP" and network['authtype'] == 'OPEN':
                ...
                for cmd in cmd_list:
                    ...
                    misc.Run(cmd)
=======
>>>>>>> /work/joliebig/workspace_sple/featurehouse/fstmerge/fstmerge_tmp1309793773088/fstmerge_var2_2693634860966632819
