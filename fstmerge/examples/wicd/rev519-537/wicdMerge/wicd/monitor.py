...
class  ConnectionStatus (object) :
	...
	def check_for_wireless_connection(self, wireless_ip):
        ...
        if wifi_signal == 0:
            ...
            self.connection_lost_counter = 0
        if (wifi_signal != self.last_strength or            self.network != wireless.GetCurrentNetwork(self.iwconfig)):
	...
	def update_state(self, state, wired_ip=None, wifi_ip=None):
        ...
        if state == misc.NOT_CONNECTED:
            ...
            self.reconnect_tries = 0
            info = [wifi_ip, wireless.GetCurrentNetwork(iwconfig),                    str(self._get_printable_sig_strength()),                    str(wireless.GetCurrentNetworkID(iwconfig))]
