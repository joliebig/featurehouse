...
class  AdvancedSettingsDialog (gtk.Dialog) :
	def __init__(self):
        ...
        """
        gtk.Dialog.__init__(self, title=language['advanced_settings'],                            flags=gtk.DIALOG_MODAL, buttons=(gtk.STOCK_CANCEL,                                                           gtk.RESPONSE_REJECT,                                                           gtk.STOCK_OK,                                                           gtk.RESPONSE_ACCEPT))
	...
	def toggle_dns_checkbox(self, widget=None):
        ...
        if self.chkbox_static_ip.get_active():
            self.chkbox_static_dns.set_active(self.chkbox_static_ip.                                                            get_active())
            self.chkbox_static_dns.set_sensitive(False)
        self.chkbox_global_dns.set_sensitive(self.chkbox_static_dns.                                                            get_active())
...
class  WirelessSettingsDialog (AdvancedSettingsDialog) :
	...
	def set_values(self):
        ...
        self.chkbox_global_dns.set_active(bool(wireless.GetWirelessProperty(networkID,                                                                  'use_global_dns')))
        ...
        self.reset_static_checkboxes()
        self.chkbox_encryption.set_active(bool(wireless.GetWirelessProperty(networkID,                                                                       'encryption')))
        self.chkbox_global_settings.set_active(bool(wireless.GetWirelessProperty(networkID,                                                             'use_settings_globally')))
        activeID = -1
	...
	def change_encrypt_method(self, widget=None):
        ...
        for x in opts:
            ...
            box.entry.set_text(noneToBlankString(                wireless.GetWirelessProperty(self.networkID, opts[x][1])))
...
class  WiredNetworkEntry (NetworkEntry) :
	def __init__(self, dbus_ifaces):
        ...
        self.chkbox_default_profile.connect("toggled",                                            self.toggle_default_profile)
        ...
        if os.getuid() != 0:
            try:
                ...
                if sudo_prog.endswith("gksu") or sudo_prog.endswith("ktsuss"):
                    ...
                    msg_flag = "--caption"
                misc.LaunchAndWait([sudo_prog, msg_flag, msg,                                    wpath.lib + "configscript.py", profile,                                    "wired"])
	...
	def remove_profile(self, widget):
        ...
        self.combo_profile_names.remove_text(self.combo_profile_names.                                                                 get_active())
        self.combo_profile_names.set_active(0)
	...
	def toggle_default_profile(self, widget):
        ...
        wired.SetWiredProperty("default",                               self.chkbox_default_profile.get_active())
...
class  WirelessNetworkEntry (NetworkEntry) :
	def __init__(self, networkID, dbus_ifaces):
        ...
        self.set_signal_strength(wireless.GetWirelessProperty(networkID,                                                              'quality'),                                 wireless.GetWirelessProperty(networkID,                                                              'strength'))
        ...
        self.set_encryption(wireless.GetWirelessProperty(networkID,                                                         'encryption'),                            wireless.GetWirelessProperty(networkID,                                                        'encryption_method'))
        self.expander.set_use_markup(True)
        self.expander.set_label(self._escape(self.essid) + "   " +                                self.lbl_strength.get_label() + "   " +                                self.lbl_encryption.get_label() + "   " +                                self.lbl_mac.get_label())
        ...
        if os.getuid() != 0:
            try:
                ...
                if sudo_prog.endswith("gksu") or sudo_prog.endswith("ktsuss"):
                    ...
                    msg_flag = "--caption"
                misc.LaunchAndWait([sudo_prog, msg_flag, msg,                                    wpath.lib + "configscript.py",                                    str(self.networkID), "wireless"])
            except IOError:
                error(None, "Could not find a graphical sudo program." + \
                      "  Script editor could no be launched.")
        else:
            misc.LaunchAndWait(["./configscript.py", str(self.networkID),                               "wireless"])
    
	def update_autoconnect(self, widget=None):
        """ Called when the autoconnect checkbox is toggled. """
        wireless.SetWirelessProperty(self.networkID, "automatic",                                     noneToString(self.chkbox_autoconnect.                                                                  get_active()))
