...
public abstract class Image extends Rectangle {
    ...
    public void simplifyColorspace() {
        ...
        if (cs.isName())
            ...
            PdfObject first = ((PdfArray)value).getArrayList().get(0);
            if (PdfName.INDEXED.equals(first)) {
                ArrayList<PdfObject> array = ((PdfArray)value).getArrayList();
                if (array.size() >= 2 && array.get(1).isArray()) {
                     array.set(1, simplifyColorspace(array.get(1)));
                }
            }
        }
    ...
    private PdfObject simplifyColorspace(PdfObject obj) {
        if (obj == null || !obj.isArray())
            return obj;
        PdfObject first = ((PdfArray)obj).getArrayList().get(0);
