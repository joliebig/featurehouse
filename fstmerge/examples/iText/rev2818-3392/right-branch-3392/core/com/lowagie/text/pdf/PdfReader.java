...
public class PdfReader implements PdfViewerPreferences {
    ...
    public static Rectangle getNormalizedRectangle(PdfArray box) {
        ArrayList<PdfObject> rect = box.getArrayList();
        float llx = ((PdfNumber)getPdfObjectRelease(rect.get(0))).floatValue();
        float lly = ((PdfNumber)getPdfObjectRelease(rect.get(1))).floatValue();
        float urx = ((PdfNumber)getPdfObjectRelease(rect.get(2))).floatValue();
        float ury = ((PdfNumber)getPdfObjectRelease(rect.get(3))).floatValue();
    ...
    private void readDecryptedDocObj() throws IOException {
        ...
        if (documentIDs != null) {
            o = documentIDs.getArrayList().get(0);
            strings.remove(o);
            s = o.toString();
        ...
        {                    
            ...
            {
                PdfObject recipient = recipients.getArrayList().get(i);
            ...
            try {
                ...
                for (int i=0; i<recipients.size(); i++)
                {
                  byte[] encodedRecipient = recipients.getArrayList().get(i).getBytes();  
    ...
    protected void readDocObjPartial() throws IOException {
        ...
        xrefObj.addAll(Collections.nCopies(xref.length / 2, (PdfObject)null));
        readDecryptedDocObj();
    ...
    protected boolean readXRefStream(int ptr) throws IOException {
        ...
        int bptr = 0;
        ArrayList<PdfObject> wa = w.getArrayList();
        int wc[] = new int[3];
        for (int k = 0; k < 3; ++k)
            wc[k] = ((PdfNumber)wa.get(k)).intValue();
        ArrayList<PdfObject> sections = index.getArrayList();
    ...
    public byte[] getPageContent(int pageNum, RandomAccessFileOrArray file) throws IOException{
        ...
        if (contents.isStream()) {
            ...
            for (int k = 0; k < list.size(); ++k) {
                PdfObject item = getPdfObjectRelease(list.get(k));
    ...
    protected void killXref(PdfObject obj) {
        ...
        switch (obj.type()) {
            ...
            case PdfObject.ARRAY: {
                ArrayList<PdfObject> t = ((PdfArray)obj).getArrayList();
                for (PdfObject o: t)
                    killXref(o);
                break;
            }
    ...
    public int shuffleSubsetNames() {
        ...
        for (int k = 1; k < xrefObj.size(); ++k) {
            ...
            if (existsName(dic, PdfName.SUBTYPE, PdfName.TYPE1)
                ...
                if (arr == null)
                    continue;
                ArrayList<PdfObject> list = arr.getArrayList();
                if (list.isEmpty())
                    continue;
                PdfDictionary desc = (PdfDictionary)getPdfObject(list.get(0));
    ...
    public HashMap<String, PdfObject> getNamedDestination() {
        HashMap<String, PdfObject> names = getNamedDestinationFromNames();
    ...
    public HashMap<String, PdfObject> getNamedDestinationFromNames() {
        HashMap<String, PdfObject> names = new HashMap<String, PdfObject>();
    ...
    private boolean replaceNamedDestination(PdfObject obj, HashMap<String, PdfObject> names) {
    ...
    public void removeFields() {
        ...
        for (int k = 1; k <= pageRefs.size(); ++k) {
            ...
            for (int j = 0; j < arr.size(); ++j) {
                PdfObject obj = getPdfObjectRelease(arr.get(j));
                if (obj == null || !obj.isDictionary())
                    continue;
    ...
    public ArrayList<PdfAnnotation.PdfImportedLink> getLinks(int page) {
        ...
        if (pageDic.get(PdfName.ANNOTS) != null) {
            ...
            ArrayList<PdfObject> arr = annots.getArrayList();
            for (int j = 0; j < arr.size(); ++j) {
                PdfDictionary annot = (PdfDictionary)getPdfObjectRelease(arr.get(j));
    ...
    private void iterateBookmarks(PdfObject outlineRef, HashMap<String, PdfObject> names) {
    ...
    public void consolidateNamedDestinations() {
        ...
        HashMap<String, PdfObject> names = getNamedDestination();
        if (names.isEmpty())
            return;
        for (int k = 1; k <= pageRefs.size(); ++k) {
            PdfDictionary page = pageRefs.getPageN(k);
            ...
            ArrayList<PdfObject> list = annots.getArrayList();
            boolean commitAnnots = false;
            for (int an = 0; an < list.size(); ++an) {
                PdfObject objRef = list.get(an);
    ...
    protected static PdfObject duplicatePdfObject(PdfObject original, PdfReader newReader) {
        ...
        switch (original.type()) {
            ...
            case PdfObject.ARRAY: {
                ArrayList<PdfObject> list = ((PdfArray)original).getArrayList();
                PdfArray arr = new PdfArray();
                for (PdfObject o: list) {
                    arr.add(duplicatePdfObject(o, newReader));
                }
                return arr;
            }
            case PdfObject.INDIRECT: {
    ...
    public int getCertificationLevel() {
        ...
        if (arr == null || arr.size() == 0)
            return PdfSignatureAppearance.NOT_CERTIFIED;
        dic = (PdfDictionary)getPdfObject(arr.getArrayList().get(0));
        if (dic == null)
            return PdfSignatureAppearance.NOT_CERTIFIED;
