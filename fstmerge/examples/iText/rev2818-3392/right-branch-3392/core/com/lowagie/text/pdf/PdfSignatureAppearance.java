...
public class PdfSignatureAppearance {
    ...
    public void setVisibleSignature(String fieldName) {
        ...
        ArrayList<PdfObject> ar = r.getArrayList();
        float llx = ((PdfNumber)PdfReader.getPdfObject(ar.get(0))).floatValue();
        float lly = ((PdfNumber)PdfReader.getPdfObject(ar.get(1))).floatValue();
        float urx = ((PdfNumber)PdfReader.getPdfObject(ar.get(2))).floatValue();
        float ury = ((PdfNumber)PdfReader.getPdfObject(ar.get(3))).floatValue();
        pageRect = new Rectangle(llx, lly, urx, ury);
        pageRect.normalize();
        page = item.page.get(0).intValue();
    ...
    public void preClose(HashMap<PdfName, Integer> exclusionSizes) throws IOException, DocumentException {
        ...
        if (fieldExists) {
            ArrayList<PdfDictionary> widgets = af.getFieldItem(name).widgets;
            PdfDictionary widget = widgets.get(0);
