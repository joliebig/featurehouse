...
class Msnp(ProfileManager.ProfileManager):
    ...
    def do_login_successful(self):
        ...
        if mlCache and diCache and not self.safemode:
            ...
            try:
                ...
                soap.requests.membership(self.onGetMembershipList, sync=True)
                soap.requests.dynamic_items(self.onGetDynamicItems, sync=True)
        ...
        if not self.safemode:
            ...
            def safeADL(chunk, next):
                ...
                if next:
                    gobject.idle_add(safeADL, next.pop(0), next)
                return False
        ...
        if mlCache and diCache and not self.safemode:
            soap.requests.membership(self.onGetMembershipList, sync=False)
            soap.requests.dynamic_items(self.onGetDynamicItems, sync=False)
    ...
    def on_wink_received(self, switchboard, msnobj, data, email):
        ...
        try:
            sha1d = sha.sha(msnobj.sha1d).hexdigest()
