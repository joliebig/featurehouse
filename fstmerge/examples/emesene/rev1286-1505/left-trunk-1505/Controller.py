...
class Controller(gobject.GObject):
    ...
    def __init__(self, username, minimized, safe, leakdebug):
        ...
        self.debugManager.connect('message-added', print_and_inspect)
        self.debug('fsck you', 'core')
    ...
    def login(self, user, password, status):
        ...
        self.msn.login()
    def debug(self, message, category='?', level=DebugLevel.info, caller='', filename=''):
        ...
        if not caller:
            try:
                ...
                parent_class = ''
            caller = '%s%s' % (parent_class, caller_obj[3])
        if not filename:
            filename = caller_obj[0].f_code.co_filename
        self.debugManager.add({'message': message, 'category': category,
                               'level':level, 'caller':caller,
                               'filename':filename})
    ...
    def on_msnp_exception(self, msnp, exception):
        ...
        message = '<span weight="bold" size="larger">Exception\n</span>' \
