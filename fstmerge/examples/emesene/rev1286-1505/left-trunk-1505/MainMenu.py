...
class MainMenu(gtk.MenuBar):
    ...
    def __init__(self, controller, window_type, accelGroup):
        ...
        wikiMenuItem = self.newImageMenuItem(_('Wiki'), gtk.STOCK_YES)
        homeMenuItem = self.newImageMenuItem(_('_Homepage'), gtk.STOCK_HOME)
        debugMenuItem = self.newImageMenuItem(_('Debug console'), 
                                              gtk.STOCK_INFO) #TODO: icon
        ...
        wikiMenuItem.connect('activate', self.on_click_url, \
            'http://emesene.org/trac/', None)
        comunityMenuItem.connect('activate', self.on_click_url, \
            'http://emesene.org/smf/', None)
        debugMenuItem.connect('activate', self.on_debug_activate)
        if window_type == 'userlist':
    ...
    def on_about_activate(self, *args):
        ...
        about.set_version('trunk')
        about.set_copyright('Luis Mariano Guerra and dx')
    ...
    def on_debug_activate(self, *args):
        window = DebugWindow(self.controller.debugManager)
        window.store.connect_to_manager()
        window.show()
