...
class HtmlHandler:
    ...
    def _object_event( self, widget, event, attrs ):
        ...
        if callbacks.has_key( attrs['type'] ) and callbacks[attrs['type']][1] != None:
...
class HtmlTextView(gtk.TextView):
    ...
    def customEmoticon( self, textview, id, path=None ):
        if self.customObjects.has_key( id ):
    ...
    def customEmoticonEvent( self, img, event, attrs ):
        if not self.customObjects.has_key( attrs['class'] ):
            return
        ...
        if event.type == gtk.gdk.BUTTON_PRESS and event.button == 3:
            if path != None:
                ...
                menu_items.set_image( gtk.image_new_from_stock( gtk.STOCK_SAVE, gtk.ICON_SIZE_MENU ))
                menu.append(menu_items)
                menu_items.connect( 'activate', self.customEmoticonSave, path, attrs['data'] )
                menu_items.show()
                menu.popup(None, None, None, event.button, event.time)
    ...
    def setCustomObject( self, id, path=None, type=None ):
        if self.customObjectsCallbacks.has_key( type ):
