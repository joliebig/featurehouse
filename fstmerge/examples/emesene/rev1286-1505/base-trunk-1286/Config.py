...
DEFAULT_USER_CONFIG = {
    ...
    'statusTransformation' : 'corner|alpha', # valid values are corner, pixelate, alpha(seperated by | )
...
class Config(gobject.GObject):
    ...
    def __init__(self):
        ...
        if os.name != 'nt' or hasattr(sys, "frozen"):
            _mkdir('', PATH.HOME_DIR, '.config')
            _mkdir('Config dir', PATH.CONFIG_DIR)
            _mkdir('Theme dir in config', PATH.THEME_HOME_PATH)
            _mkdir('Smilies dir in config', PATH.SMILIES_HOME_PATH)
            _mkdir('Convthemes dir in config', PATH.CONVTHEMES_HOME_PATH)
            _mkdir('Theme dir in config', PATH.THEME_HOME_PATH)
            if _mkdir('Plugins dir in config', PATH.PLUGIN_HOME_PATH):
                shutil.copyfile(PATH.PLUGIN_SYSTEM_WIDE_PATH + '/__init__.py', \
                                PATH.PLUGIN_HOME_PATH + '/__init__.py')
        if _mkfile('Config file', PATH.CONFIG_DIR, 'config'):
    ...
    def readGlobalConfig(self):
        ...
        conf = None
        try:
            conf = open(PATH.CONFIG_DIR + '/config', 'r')
            string = conf.read()
    ...
    def writeGlobalConfig(self):
        ...
        try:
            conf = open(PATH.CONFIG_DIR + '/config', 'w')
    ...
    def setCurrentUser(self, email):
        ...
        if email != '':
            _mkdir('User config dir', PATH.CONFIG_DIR, self.currentUser)
            if _mkfile('Config file created: ', PATH.CONFIG_DIR, \
               self.currentUser, 'config'):
                self.user = ConfigDict(self, self.writeUserConfig, \
                    DEFAULT_USER_CONFIG)
                self.writeUserConfig()
            _mkdir('', PATH.CONFIG_DIR, self.currentUser, 'logs')
            _mkdir('', PATH.CONFIG_DIR, self.currentUser, 'cache')
            _mkdir('', PATH.CONFIG_DIR, self.currentUser, 'avatars')
            _mkdir('', PATH.CONFIG_DIR, self.currentUser, 'custom_emoticons')
            self.readUserConfig()
        else:
            self.user = {}
    ...
    def readUserConfig(self):
        ...
        conf = None
        try:
            conf = open(PATH.CONFIG_DIR + '/' + self.currentUser + '/config', 'r')
            string = conf.read()
            for i in string.splitlines():
                if i != '':
                    try:
    ...
    def writeUserConfig(self):
        '''write the config to the file, overwrite current config file'''
        try:
            conf = open(PATH.CONFIG_DIR + '/' + self.currentUser + '/config', 'w')
    ...
    def getThemes(self):
        ...
        return self._explorePath(PATH.THEME_HOME_PATH) + \
               self._explorePath(PATH.THEME_SYSTEM_WIDE_PATH)
    def getSmilieThemes(self):
        '''the same with smilie themes'''
        return self._explorePath(PATH.SMILIES_HOME_PATH) + \
               self._explorePath(PATH.SMILIES_SYSTEM_WIDE_PATH)
    def getLogPath(self):
        logPath = self.user['logPath']
        if logPath != '':
            return logPath
        else:
            return PATH.CONFIG_DIR + os.sep + self.currentUser + os.sep + 'logs'
    def getCachePath(self):
        return PATH.CONFIG_DIR + os.sep + self.currentUser + os.sep + 'cache'
    def getUserConfigPath(self):
        return PATH.CONFIG_DIR + os.sep + self.currentUser
    def getAvatarsCachePath(self):
        return PATH.CONFIG_DIR + os.sep + self.currentUser + os.sep + 'avatars'
    def getCustomEmoticonsPath(self):
        return PATH.CONFIG_DIR + os.sep + self.currentUser + os.sep + 'custom_emoticons'
    def getConversationThemesPath(self):
        return PATH.CONFIG_DIR + os.sep + 'conversation_themes'
