...
class avatarHolder( gtk.Widget ):
    ...
    def do_expose_event(self, evnt):
        ...
        if self.inAnimation :
            self.__draw( ctx , cell_area , self._pixbuf, 1 - (float(self.currentFrame) / self.totalFrames))                        
            self.__draw( ctx , cell_area , self.transitionPixbuf,(float(self.currentFrame) / self.totalFrames))
        else:
            self.__draw( ctx , cell_area , self._pixbuf, 1)
    def __shouldReplace(self,pixbuf):
        if (self._pixbuf != None) and (pixbuf != None) and pixbuf.get_pixels() == self._pixbuf.get_pixels():         
            return False
        else: 
            return True
...
class WidgetToggleBox(gtk.Widget):
    ...
    def do_realize(self):
        ...
        if gtk.gtk_version >= (2, 12, 0) and self.description:
